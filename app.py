...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v6.3", page_icon="🎣")
...
st.markdown("""
    ...
    /* 列ごとの調整 */
    .col-time { width: 15%; font-weight: bold; }
    .col-honmei { width: 20%; color: #d63031; font-weight: bold; }
    .col-osae { width: 20%; color: #0984e3; }
    .col-tac { width: 15%; font-size: 12px; }
    .col-note { width: 30%; font-size: 11px; text-align: left !important; color: #666; }
    
    /* スマホでの微調整 */
    @media screen and (max-width: 600px) {
        table.matsuri-table { font-size: 11px; }
        .col-honmei, .col-osae { font-size: 12px; }
    }
    </style>
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況・気象・ベイトパターンを解析中...'):
            ...
            for h in range(5, 16):
                ...
                time_weather_display = f"{h}:00<br>{w_icon} {wind_text}"
                tactics_display = f"{spd}・{hk}"
                if worm: tactics_display += f"<br>{worm}"
                ...
                table_rows.append(f"""
                <tr>
                    <td class="col-time">{time_weather_display}</td>
                    <td class="col-honmei">{tie1}</td>
                    <td class="col-osae">{tie2}</td>
                    <td class="col-tac">{tactics_display}</td>
            ...
            # 【変更点】DataFrameではなく、HTMLテーブルを構築して表示する
            table_html = f"""
