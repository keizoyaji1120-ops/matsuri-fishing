...
st.set_page_config(page_title="魔釣 - 瀬戸内タイラバ予報 v7.4", page_icon="🎣")
...
def suggest_strategy(h, sun_h, sc, t_diff, month, temp, cloud_cover, rain, depth):
    ...
    if rain >= 0.5:
        ...
        c1 = "チャート" if h % 2 == 0 else "赤ゼブラ"
    # 【追加】水深によるカラー補正
    elif depth >= 60: # 鳴門などディープ (光が届きにくいためシルエット重視)
        c1 = "オレンジゼブラ" if h % 2 == 0 else "赤黒"
    elif depth <= 30: # 小豆島・瀬戸大橋などシャロー (フラッシング重視)
    ...
    if rain >= 0.5:
        ...
        s1 = "ロングカーリー"
    # 【追加】水深による形状補正
    elif depth >= 60: # 鳴門などディープ (水押し・アピール重視)
        s1 = "強波動" if sc >= 40 else "カーリー"
    elif depth <= 30: # 小豆島・瀬戸大橋などシャロー (スモールシルエット重視)
...
def main():
    ...
    depth = AREA_OPTIONS[selected_area]["depth"] # 【追加】エリアごとの想定水深を取得
        ...
        try:
            with st.spinner(f'{selected_area}の海況・気象・水深パターンを解析中...'):
                ...
                for h in range(5, 16):
                    ...
                    # 【変更】関数呼び出し時に depth(水深) を渡して判定させる
