...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v6.0", page_icon="🎣")
...
def get_seasonal_bait(month):
    if month in [12, 1, 2]:
        ...
        return "イカナゴ", "ミドキン・緑・オレ金" # 【変更】ミドキン追加
    elif month in [6, 7]:
...
def suggest_strategy(h, sun_h, sc, t_diff, month, temp, cloud_cover, rain):
    ...
    if rain >= 0.5:
        ...
        c1 = "チャート" if h % 2 == 0 else "赤ゼブラ" # 【変更】朝マズメに赤ゼブラ追加
    elif t_diff <= -0.1:
        c1 = "コーラ" if h % 2 == 0 else "赤黒"
    elif month in [3, 4, 5]:
        c1 = "ミドキン" if h % 2 == 0 else "グリーン" # 【変更】春はミドキン本命
    elif is_nori_season and (temp < 13.0 or sc < 45):
        c1 = "黒/海苔" if h % 2 == 0 else "コーラ"
    elif cloud_cover > 70:
        c1 = "チャート" if h % 2 == 0 else "赤ゼブラ" # 【変更】曇天に赤ゼブラ
    elif h <= sun_h + 2:
    ...
    elif c1 == "ミドキン": c2 = "オレ金" # ミドキンの抑えはオレ金
    elif c1 == "オレ金": c2 = "ピンク"
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況・気象・ベイトパターンを解析中...'):
            ...
            for h in range(5, 16):
                ...
                time_weather_display = f"{h}:00 {w_icon}\n{wind_text}"
                ...
                data_rows.append({
                    "時間(天気・風速)": time_weather_display,
                    "本命": tie1,
                    "抑え": tie2,
                    "巻き速度・針": tactics_display,
                    "備考": " ".join(notes)
                })
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "時間(天気・風速)": st.column_config.TextColumn("時間(天気・風速)", width="medium"),
                    "本命": st.column_config.TextColumn("本命", width="medium"),
                    "抑え": st.column_config.TextColumn("抑え", width="medium"),
                    "巻き速度・針": st.column_config.TextColumn("巻き速度・針", width="medium"),
