...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v5.4", page_icon="🎣")

# --- ソート無効化のためのCSS ---
st.markdown("""
    <style>
    /* データフレームのヘッダーをクリック不可にする */
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況・気象・ベイトパターンを解析中...'):
            ...
            for h in range(5, 16):
                ...
                if wind >= 10.0:
                    sc = 0
                    wind_text = "爆風" # 10m以上 (中止)
                elif wind >= 7.0:
                    sc -= 10 # 7.0-9.9m (強風・底取り難・減点)
                    wind_text = "強風" 
                elif wind >= 5.0:
                    sc += 5 # 5.0-6.9m (やや強・活性良・微加点)
                    wind_text = "やや強"
                elif wind >= 2.0:
                    sc += 20 # 2.0-4.9m (最適・ドテラ最高・大幅加点)
                    wind_text = "最適"
                else:
                    sc -= 20 # 0.0-1.9m (静穏・地獄の凪・大幅減点)
                    wind_text = "静穏"
