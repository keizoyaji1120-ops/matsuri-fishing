...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v3.7", page_icon="🎣")
...
def suggest_strategy(h, sun_h, sc, t_diff, month, temp):
    # --- 1. 本命パターン (Best) ---
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況データを解析中...'):
            ...
            for h in range(5, 16):
                ...
                # 【変更点】絶対水温によるペナルティ (低水温補正)
                low_temp_alert = ""
                if ct <= 10.0:
                    sc = int(sc * 0.2) # 10度以下は激渋(2割)
                    low_temp_alert = "⚠️低水温(激渋)"
                elif ct <= 12.0:
                    sc = int(sc * 0.5) # 12度以下は要注意(5割)
                    low_temp_alert = "⚠️低水温"
                
                if sc < 0: sc = 0
                
                tie1, tie2 = suggest_strategy(h, sun_h, sc, tdiff, target_date.month, ct)
                
                time_str = f"{h}:00"
                note = ""
                if slack: note += "★転流 "
                if low_temp_alert: note += low_temp_alert
