...
def suggest_strategy(h, sun_h, sc, t_diff):
    ...
    c1 = "赤オレ"; s1 = "極細"
    if h <= sun_h: c1 = "チャート"
    elif h <= sun_h + 1: c1 = "オレ金"
    elif t_diff <= -0.1: c1 = "黒/海苔"
...
def main():
    ...
    st.markdown("---")
    st.markdown("▼ 公式の潮流予測（正確な時間）はこちらで確認")
    st.link_button("海上保安庁の潮流データを開く", KAIHO_URL)
    st.markdown("---")
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況データを解析中...'):
            ...
            st.subheader("📝 時間別 推奨ネクタイ(2種)")
            df = pd.DataFrame(data_rows)
            st.dataframe(
                df, 
                use_container_width=True,
                column_config={
                    "推奨ネクタイ": st.column_config.TextColumn(
                        "推奨ネクタイ (①本命 ②抑え)",
                        help="①がメイン、②がローテーション用です",
                        width="medium"
                    )
                }
            )

            st.caption(f"※潮位グラフは推測値です。正確な転流・最強流の時刻は、上の「海上保安庁ボタン」から確認することをおすすめします。")
