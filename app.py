...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v3.9", page_icon="🎣")
...
def suggest_strategy(h, sun_h, sc, t_diff, month, temp, cloud_cover):
    # --- 1. 本命パターン (Best) ---
    ...
    if h <= sun_h:
        ...
        # 【追加】曇り・雨などローライト時 -> チャートやグロー系が強くなる
...
def get_weather_data(target_date):
    ...
    # 【変更点】天気予報APIに「雲量(cloud_cover)」と「風速(wind_speed_10m)」を追加
    p_weather = {
        "latitude": AKASHI_LAT, 
        "longitude": AKASHI_LON, 
        "daily": "sunrise", 
        "hourly": "cloud_cover,wind_speed_10m", # ここを追加
        "start_date": d_str, 
        "end_date": d_str, 
        "timezone": "Asia/Tokyo"
    }
...
def main():
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況・気象データを解析中...'):
            ...
            # 天気・風速データの取得 (予報がない遠い日付の場合はNoneになるので対策)
            ...
            for h in range(5, 16):
                ...
                # 【追加機能】天気と風によるスコア補正
                w_note = ""
                # 1. 雲量補正 (曇りや雨はプレッシャー低下でプラス)
                if cloud >= 60: 
                    sc += 10
                    w_note += "☁️"
                elif cloud <= 20: # 快晴はプレッシャー高い
                    sc -= 5
                    w_note += "☀️"
                else:
                    w_note += "⛅"

                # 2. 風速補正 (適度な風はプラス、無風と爆風はマイナス)
                if wind >= 10.0:
                    sc = 0 # 危険すぎるためスコア0
                    w_note += " 🌪️激風"
                elif wind >= 7.0: # 少し強いが流れる
                    sc += 5
                    w_note += f" 🌬️{wind}m"
                elif wind >= 3.0: # ベストな風
                    sc += 15
                    w_note += f" 🍃{wind}m"
                elif wind <= 1.5: # ほぼ無風(流れない)
                    sc -= 15
                    w_note += " 凪"
                else:
                    w_note += f" {wind}m"
                ...
                # 戦略提案 (cloud情報を渡す)
                tie1, tie2 = suggest_strategy(h, sun_h, sc, tdiff, target_date.month, ct, cloud)
                ...
                # 備考欄の結合
                final_note = f"{w_note} | " + " ".join(notes)
