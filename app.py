...
st.set_page_config(page_title="魔釣 - 瀬戸内タイラバ予報 v7.2", page_icon="🎣")
    ...
    if st.button("魔釣予報を開始する"):
        try:
            with st.spinner(f'{selected_area}の海況・気象・ベイトパターンを解析中...'):
                ...
                st.markdown("### 📺 タイラバ必見！ビンソルCHANNEL")
                st.markdown("ジャッカル・ビンビンスイッチの最新テクニックやタイラバの極意を動画でチェック！")
                st.video("https://www.youtube.com/watch?v=yIEz-MgHLbw")
                st.caption("▲ 【タイラバ】ビンビンスイッチの今！マダイ攻略最前線 / 田中亜衣")
