...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v5.1", page_icon="🎣")
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況・気象・ベイトパターンを解析中...'):
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "時間": st.column_config.TextColumn("時間(天気)", width="small"),
                    "本命": st.column_config.TextColumn("本命", width="medium"),
                    "抑え": st.column_config.TextColumn("抑え", width="medium"),
                    "戦術": st.column_config.TextColumn("戦術(速/針)", width="medium"),
