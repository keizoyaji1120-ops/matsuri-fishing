...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v7.0", page_icon="🎣", layout="wide")
...
def get_weather_data(target_date):
    ...
    # 【バグ修正】timezone="Asia/Tokyo" を追加し、日本時間ベースでの水温データを正確に取得
    p_temp = {
        "latitude": AKASHI_LAT, 
...
def main():
    st.markdown("""
        <h1 style='text-align: center; font-size: 38px; margin-bottom: 5px; font-weight: 800;'>
            ...
            <span style='font-size: 24px; font-weight: normal; color: #555;'>明石タイラバ予報</span>
        </h1>
    """, unsafe_allow_html=True)
    ...
    st.info(f"🐟 **現在のシーズナルパターン: {bait_name}** （有効カラー目安: {bait_colors}）")

    if st.button("魔釣予報を開始する", use_container_width=True):
        try:
            with st.spinner('明石の海況・気象・ベイトパターンをAI解析中...'):
                ...
                # APIからのデータ欠損ガード
                if not wd or "daily" not in wd or "sunrise" not in wd["daily"]:
                    sun_h = 6
                else:
                ...
                # 【バグ修正】JSTベースになったためオフセットを15から24に変更 (0~23が前日、24~47が当日)
                OFF = 24 
                ...
                s_col1.info(f"**水深30m**: {s30}")
                s_col2.info(f"**水深45m**: {s45}")
                s_col3.info(f"**水深60m**: {s60}")
                ...
                plt.title("Time vs Score / Temp / Tide", fontsize=TITLE_SIZE, fontweight='bold', color='#2c3e50')
                ...
                with col_link1:
                    st.link_button("🌊 海上保安庁の潮流情報", KAIHO_URL, use_container_width=True)
                    
                with col_link2:
                    st.link_button("🚤 どこの釣り座が釣れる？", SEAT_CHECKER_URL, use_container_width=True)
