...
st.set_page_config(page_title="魔釣 - 明石タイラバ予報 v5.2", page_icon="🎣")
...
def suggest_strategy(h, sun_h, sc, t_diff, month, temp, cloud_cover, rain):
    ...
    # --- 【新機能】フックサイズ 5段階判定 (3S, SS, S, M, L) ---
    # 1. 3S (Micro): 冬場の海苔パターン、極寒期
    if month in [12, 1, 2] and (temp < 10.0 or sc < 30):
        hook = "3S"
    # 2. SS (Super Short): 春のイカナゴパターン初期
    elif month in [3, 4]:
        hook = "SS"
    # 3. S (Short): 活性が低い時、デッドスロー時
    elif speed == "激遅" or sc < 40:
        hook = "S"
    # 4. L (Large): 高水温、高活性、夏〜秋のイワシパターン
    ...
    if st.button("魔釣予報を開始する"):
        with st.spinner('明石の海況・気象・ベイトパターンを解析中...'):
            ...
            for h in range(5, 16):
                ...
                # 【修正点】時間と天気風を結合しつつ、表示幅を確保
                time_display = f"{h}:00 {w_icon}{wind_icon}"
                
                # 【修正点】戦術を結合 (例: 早巻・L)
                tactics_display = f"{spd}・{hk}"
            ...
            # 【修正点】カラム設定で幅を「medium」以上に設定し、文字切れを防止
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "時間": st.column_config.TextColumn("時間(天気)", width="medium"), # 幅をmediumへ
                    "本命": st.column_config.TextColumn("本命", width="medium"),
                    "抑え": st.column_config.TextColumn("抑え", width="medium"),
                    "巻き速度・針": st.column_config.TextColumn("巻き速度・針", width="medium"), # 名前を変更し幅確保
                    "備考": st.column_config.TextColumn("備考", width="large"), # 幅をlargeへ
                }
            )
